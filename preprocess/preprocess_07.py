...
RAW_PATH = os.path.join(BASE_DIR, "07._주중주말_서비스인구.csv")
OUT_PATH = os.path.join(BASE_DIR, "07._주중주말_서비스인구_preprocessed.csv")
...
hw_dist = df["hw"].value_counts()
print(f"\n  hw 분포: {hw_dist.to_dict()}")

coord_missing = df[["lon", "lat"]].isna().any(axis=1).sum()
print(f"  좌표(lon/lat) 결측 행: {coord_missing:,}")
...
# ── 8. 전처리 후 기본 통계 ───────────────────────────────────
print(f"\n[전처리 후 기본 통계]")
print(df[pop_cols].describe().round(6).to_string())

print(f"\n[hw별 기본 통계]")
for hw_val in df["hw"].unique():
    sub = df[df["hw"] == hw_val]
    label = "주중(H)" if hw_val == "H" else "주말(W)"
    print(f"\n  {label} ({len(sub):,}행):")
    print(sub[pop_cols].describe().round(6).to_string().replace("\n", "\n    "))
