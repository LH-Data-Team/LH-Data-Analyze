...
RAW_PATH = os.path.join(BASE_DIR, "04._성연령별_유동인구.csv")
OUT_PATH = os.path.join(BASE_DIR, "04._성연령별_유동인구_preprocessed.csv")
...
use_cols = ["gbn", "STD_YM"] + pop_cols + ["lon", "lat"]
df = df[use_cols].copy()
print(f"\n[컬럼 선택] {use_cols}")
...
print(f"  모든 인구 컬럼이 결측인 행: {all_missing_rows:,} / {len(df):,}")
print(f"\n  컬럼별 결측률:")
for c in pop_cols:
    n = df[c].isna().sum()
...
for c in pop_cols:
    ...
    print(f"  {c:15s}: 비영 {len(nonzero):>6,}건 | "
          f"Q1={q1:>8.4f}  Q3={q3:>8.4f}  상한={upper:>10.4f} | "
          f"이상치={n_outlier:>5}건  max={nonzero.max():.4f}")
...
# ── 8. 전처리 후 기본 통계 ───────────────────────────────────
print(f"\n[전처리 후 기본 통계]")
print(df[pop_cols].describe().round(4).to_string())
