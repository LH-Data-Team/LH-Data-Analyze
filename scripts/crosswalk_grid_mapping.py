"""
횡단보도 위치정보를 격자에 매핑하는 스크립트
- 입력: 01._격자_(4개_시·구).geojson, 18._횡단보도_위치정보.csv
- 출력: output/횡단보도_격자매핑.csv
"""
...
def map_crosswalks_to_grid(crosswalk_data, tree, polygons, gid_list, link_tree, link_ids):
    """횡단보도 지점을 격자에 매핑"""
    print("횡단보도 데이터 격자 매핑 중...")
    
    total = len(crosswalk_data)
    mapped_count = 0
    unmapped_count = 0
    
    for i, row in enumerate(crosswalk_data):
        lon = float(row['lon'])
        lat = float(row['lat'])
        point = Point(lon, lat)
...
def main():
    ...
    print(f"횡단보도 개수: {len(crosswalk_data):,}개\n")
    ...
    # 4. 매핑 수행
    result = map_crosswalks_to_grid(crosswalk_data, tree, polygons, gid_list, link_tree, link_ids)
