"""
유치원현황 데이터를 격자에 매핑하는 스크립트
- 입력: 01._격자_(4개_시·구).geojson, 16._유치원현황.csv
- 출력: output/유치원현황_격자매핑.csv
- 추가: age3_cnt + age4_cnt + age5_cnt 합계 컬럼 (total_age_cnt)
"""
...
KINDERGARTEN_FILE = os.path.join(DATA_DIR, '16._유치원현황.csv')
OUTPUT_DIR = os.path.join(BASE_DIR, 'output')
OUTPUT_FILE = os.path.join(OUTPUT_DIR, '16._유치원현황_격자매핑.csv')
...
def map_data_to_grid(data, tree, polygons, gid_list, link_tree, link_ids):
    """데이터 지점을 격자에 매핑하고 연령별 합계 계산"""
    print("유치원 데이터 격자 매핑 중...")
    ...
    for i, row in enumerate(data):
        ...
        # 연령별 원아수 합계 계산
        age3 = int(row['age3_cnt']) if row['age3_cnt'] else 0
        age4 = int(row['age4_cnt']) if row['age4_cnt'] else 0
        age5 = int(row['age5_cnt']) if row['age5_cnt'] else 0
        row['total_age_cnt'] = age3 + age4 + age5
    ...
    print(f"\n연령별 원아수 합계:")
    print(f"  - age3_cnt 합계: {total_age3:,}명")
    print(f"  - age4_cnt 합계: {total_age4:,}명")
    print(f"  - age5_cnt 합계: {total_age5:,}명")
    print(f"  - 전체 합계: {total_age3 + total_age4 + total_age5:,}명")
...
def main():
    ...
    print(f"유치원 개수: {len(kindergarten_data):,}개\n")
    ...
    # 4. 매핑 수행
    result = map_data_to_grid(kindergarten_data, tree, polygons, gid_list, link_tree, link_ids)

    # 5. 결과 저장 (total_age_cnt, grid_gid, link_id 컬럼 추가)
