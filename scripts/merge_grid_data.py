...
OUTPUT_FILE_SPEEDBUMP = os.path.join(OUTPUT_DIR, '격자별_통합데이터_과속방지턱순.csv')
...
def main():
    ...
    # accident_cnt 높은 순으로 정렬
    result.sort(key=lambda x: int(x['accident_cnt']), reverse=True)
    ...
    # speedbump_cnt 높은 순, 동일 시 accident_cnt 높은 순 정렬하여 별도 저장
    result_speedbump = sorted(
        result,
        key=lambda x: (int(x['speedbump_cnt']), int(x['accident_cnt'])),
        reverse=True
    )
    save_csv(result_speedbump, fieldnames, OUTPUT_FILE_SPEEDBUMP)
    print(f"과속방지턱 순 정렬 저장: {OUTPUT_FILE_SPEEDBUMP}")
